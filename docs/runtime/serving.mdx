...
rayai.serve(MyAgent, name="my-agent", num_cpus=1, memory="2GB")
```
...
| `route_prefix` | str | /{name} | URL prefix for this agent |
...
The `rayai up` command discovers and runs all agents and MCP servers in your project:
...
Serving at http://localhost:8000
  ...
  Agents:
    POST http://localhost:8000/myagent/
...
3. Collects `rayai.serve()` and `rayai.serve_mcp()` registrations
...
- **Agent endpoints**: `http://localhost:8000/{agent_name}/`
- **Health check**: `http://localhost:8000/{agent_name}/health`
