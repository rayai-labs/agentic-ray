...
The `@rayai.tool` decorator converts Python functions into async [Ray tasks](https://docs.ray.io/en/latest/ray-core/tasks.html) that execute across your cluster.
...
Use `rayai.tool()` as a wrapper to run framework tools on Ray:
