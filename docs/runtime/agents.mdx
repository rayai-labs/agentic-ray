...
rayai.serve(MyAgent, name="my-agent", num_cpus=2, memory="4GB", replicas=3)
```
...
Configure resources when serving with `rayai.serve()`:
...
<Info>
For framework-based agents, pass your agent directly to `rayai.serve()` - no base class needed.
</Info>