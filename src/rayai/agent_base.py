...
Example:
    ...
    # Serve the agent
    rayai.serve(MyAgent(), name="my-agent")
"""
...
class Agent(ABC):
    ...
    Attributes:
        tools: List of @rayai.tool decorated functions available to the agent.
    """
    ...
    async def _execute_tool(self, tool: Any, **kwargs: Any) -> Any:
        ...
        Returns:
            The tool's result.
        """
        # Check if it's a rayai tool (async)
        if getattr(tool, "_rayai_tool", False):
